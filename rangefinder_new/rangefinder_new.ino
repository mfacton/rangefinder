...
void b2Held() {
  if (!b2Hold) {
    ...
    String s = ((String)res).substring(0, 4);
    s.replace(".", "");
    if (res < 10) {
      displayDecimal = true;
      setDigit(0, 11);
      setDigit(1, s.substring(0, 1).toInt());
      setDigit(2, s.substring(1, 2).toInt());
    } else {
      setDigit(0, s.substring(0, 1).toInt());
      setDigit(1, s.substring(1, 2).toInt());
      setDigit(2, s.substring(2, 3).toInt());
      if (res >= 100) {
